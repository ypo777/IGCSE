...
def showingItemsAndChooseingItems(array_one):
    ...
    moreItems = input("Do you want to choose additional items?  (Y or Q) If you want to quit during selecting items enter Q !\n")
    if moreItems == "Y":
      for c in array_one:
        getOutput = 0
        if c == "Solid State Drive":
            ...
            print("------------------------")
            showingItemsWithData(category_array.get(c))
            getOutput = choosenItem(customizedComputer, c)
...
def outputResult():
    ...
    discountCount = len(customizedComputer.keys())
    if discountCount == 4:
        discountAmount = float(totalPrice) * 0.05
        newtotalPrice = float(totalPrice) - float(discountAmount)
        newtotalPrice = "{:.2f}".format(newtotalPrice)
        ...
        print("---------------------")
        print("Total Price Before 5% Discount " + ":" + str(totalPrice))
        print("Total Price After  5% Discount " + ":" + str(newtotalPrice))
        discountAmount = "{:.3f}".format(discountAmount)
        print("\nSaved Amount : " + discountAmount)
    elif discountCount > 4 :
        discountAmount = float(totalPrice) * 0.1
        newtotalPrice = float(totalPrice) - float(discountAmount)
        newtotalPrice = "{:.2f}".format(newtotalPrice)
        ...
        for key, value in customizedComputer.items():
            print(str(key) + ":" + str(value))
        print("---------------------")
        print("Total Price Before 10% Discount " + ":" + str(totalPrice))
        print("Total Price After  10% Discount " + ":" + str(newtotalPrice))
        discountAmount = "{:.3f}".format(discountAmount)
        print("\nSaved Amount : " + str(discountAmount))
    else:
        print("---------------------")
        print("Total Cost of Your Computer")
        for key, value in customizedComputer.items():
